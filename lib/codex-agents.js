...
export function convertSubagentMarkdownToCodexToml(content, agentFilename) {
  ...
  // `model:` in the Markdown targets Claude Code/Copilot, so it is never emitted here:
  // only `codex_model:` produces a Codex `model` line. `codex_effort:` wins over `effort:`.
  const codexModel = definition.codexModel || '';
  const codexEffort = definition.codexEffort || definition.effort || '';

  const toml = [
    `${GENERATED_COMMENT_PREFIX}${agentFilename}`,
    `name = "${escapeTomlBasicString(codexName)}"`,
    `description = "${escapeTomlBasicString(description)}"`,
    ...(codexModel ? [`model = "${escapeTomlBasicString(codexModel)}"`] : []),
    ...(codexEffort ? [`model_reasoning_effort = "${escapeTomlBasicString(codexEffort)}"`] : []),
