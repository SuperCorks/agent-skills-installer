 ...
 * @returns {Promise<string|null>} Remote URL, or null when there is no origin remote
 */
export async function getOriginUrl(repoPath) {
  try {
    return await runGitCommand(['config', '--get', 'remote.origin.url'], resolvePath(repoPath));
  } catch {
    return null;
  }
}
...
async function refExists(cwd, ref) {
  try {
    await runGitCommand(['rev-parse', '--verify', `${ref}^{commit}`], cwd);
    return true;
  } catch {
    return false;
  }
...
async function getUpdateRemoteRef(cwd) {
  const branch = await runGitCommand(['rev-parse', '--abbrev-ref', 'HEAD'], cwd);

  if (branch && branch !== 'HEAD') {
    try {
      return await runGitCommand(['rev-parse', '--abbrev-ref', `${branch}@{upstream}`], cwd);
    } catch {
      const matchingRemoteBranch = `origin/${branch}`;
      if (await refExists(cwd, matchingRemoteBranch)) {
        return matchingRemoteBranch;
      }
    }
  }
...
async function fastForwardFromOrigin(cwd) {
  try {
    await runGitCommand(['fetch', 'origin'], cwd);
  } catch (error) {
    throw new Error(
      `Could not fetch latest changes from origin for "${cwd}". ` +
      `Check the remote configuration and network access, then rerun the installer.\n\n` +
      `Git output:\n${formatGitFailure(error)}`
    );
  }

  const remoteRef = await getUpdateRemoteRef(cwd);

  try {
    return await runGitCommand(['merge', '--ff-only', remoteRef], cwd);
  } catch (error) {
    throw new Error(
      `Could not update sparse checkout from ${remoteRef}. ` +
      `Commit, stash, or otherwise resolve local changes/non-fast-forward history in "${cwd}", then rerun the installer.\n\n` +
      `Git output:\n${formatGitFailure(error)}`
    );
  }
}
...
export async function updateSparseCheckout(repoPath, skillFolders, onProgress = () => {}) {
  ...
  // Update first so selected skills are applied from the latest known revision.
  onProgress('Fetching latest changes...');
  await fastForwardFromOrigin(absolutePath);
...
export async function pullUpdates(repoPath) {
  const absolutePath = resolvePath(repoPath);
  return fastForwardFromOrigin(absolutePath);
}
...
/**
 * Check which skills have local uncommitted changes.
 * @param {string} repoPath - Path to the sparse-checkout repo
 * @param {string[]} skillFolders - Skill folders to check
 * @returns {Promise<Set<string>>} Set of dirty skill folder names
 */
export async function checkSkillsForDirtyChanges(repoPath, skillFolders) {
  return checkPathsForDirtyChanges(repoPath, skillFolders);
}
...
/**
 * Check which subagents have local uncommitted changes.
 * @param {string} repoPath - Path to the sparse-checkout repo
 * @param {string[]} agentFilenames - Agent filenames to check
 * @returns {Promise<Set<string>>} Set of dirty agent filenames
 */
export async function checkSubagentsForDirtyChanges(repoPath, agentFilenames) {
  return checkPathsForDirtyChanges(repoPath, agentFilenames);
}

async function checkPathsForDirtyChanges(repoPath, paths) {
  const absolutePath = resolvePath(repoPath);
  ...
  for (const path of paths) {
    try {
      const status = await runGitCommand(['status', '--porcelain', '--', path], absolutePath);
      if (status.trim()) {
        dirtyPaths.add(path);
      }
    } catch {
      // If status fails for one path, skip it and keep checking the rest.
    }
  }
...
export async function updateSubagentsSparseCheckout(repoPath, agentFilenames, onProgress = () => {}) {
  ...
  // Update first so selected subagents are applied from the latest known revision.
  onProgress('Fetching latest changes...');
  await fastForwardFromOrigin(absolutePath);
