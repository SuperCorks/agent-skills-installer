...
import { existsSync, mkdirSync, rmSync, readdirSync, statSync, unlinkSync } from 'fs';
...
export async function sparseCloneSkills(targetPath, skillFolders, onProgress = () => {}) {
  ...
  try {
    ...
    // Remove root-level files (cone mode includes them by default)
    // We only want the skill folders, not README.md, CONTRIBUTING.md, etc.
    onProgress('Cleaning up...');
    const entries = readdirSync(absolutePath);
    for (const entry of entries) {
      const entryPath = join(absolutePath, entry);
      // Keep .git and selected skill folders, remove everything else
      if (entry !== '.git' && !skillFolders.includes(entry)) {
