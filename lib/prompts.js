...
export async function promptInstallPath(existingInstalls = [], availableSkillCount = 0) {
  ...
  const installTargets = getSkillInstallTargets();
  const standardPaths = new Set(installTargets.map(target => target.path));
  
  orderTargetsGlobalFirst(installTargets).forEach(target => {
...
export async function promptAgentInstallPath(existingInstalls = [], availableAgentCount = 0) {
  ...
  const existingByPath = new Map(existingInstalls.map(install => [install.path, install]));
  const installTargets = getAgentInstallTargets();
  const standardPaths = new Set(installTargets.map(target => target.path));
 ...
 * @param {Set<string>} dirtySkills - Skill folder names that have local uncommitted changes
 ...
 */
export async function promptSkillSelection(skills, installedSkills = [], skillsNeedingUpdate = new Set(), dirtySkills = new Set(), metadataLoader = null) {
  if (typeof dirtySkills === 'function' && metadataLoader === null) {
    metadataLoader = dirtySkills;
    dirtySkills = new Set();
  }
 ...
 * @param {Set<string>} dirtySubagents - Subagent filenames that have local uncommitted changes
 ...
 */
export async function promptSubagentSelection(subagents, installedSubagents = [], subagentsNeedingUpdate = new Set(), dirtySubagents = new Set(), metadataLoader = null) {
  if (typeof dirtySubagents === 'function' && metadataLoader === null) {
    metadataLoader = dirtySubagents;
    dirtySubagents = new Set();
  }
 ...
 * @param {Set<string>} dirtyItems - Item IDs that have local uncommitted changes
 ...
 */
function promptItemSelection(items, installedItems = [], title = '📦 Available Items', itemsNeedingUpdate = new Set(), dirtyItems = new Set(), metadataLoader = null) {
  return new Promise((resolve, reject) => {
    ...
    const render = () => {
      ...
      items.forEach((item, i) => {
        ...
        const dirtyFlag = isDirty ? ' \x1B[31m(dirty)\x1B[0m' : '';
        ...
        if (isExpanded) {
          console.log(`${highlight}${pointer} ${checkbox} ${item.name}${reset}${updateFlag}${dirtyFlag}`);
          ...
          console.log(`${highlight}${pointer} ${checkbox} ${item.name}${reset}${updateFlag}${dirtyFlag} ${highlight}${expandIcon} ${shortDesc}${reset}`);
        }
      });

      const selectedCount = selected.size;
      const updateCount = Array.from(selected).filter(id => itemsNeedingUpdate.has(id)).length;
      const dirtyCount = Array.from(selected).filter(id => dirtyItems.has(id)).length;
      const updateNote = updateCount > 0 ? ` (${updateCount} to update)` : '';
      const dirtyNote = dirtyCount > 0 ? ` (${dirtyCount} dirty)` : '';
      console.log(`\n${selectedCount} item${selectedCount !== 1 ? 's' : ''} selected${updateNote}${dirtyNote}`);
    };
