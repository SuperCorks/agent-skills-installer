...
Codex custom agents are documented as TOML files under `.codex/agents/` or `~/.codex/agents/`. The installer converts the source Markdown `.agent.md` files into Codex TOML files on install. The converter maps `name`, `description`, and the Markdown body to `developer_instructions`; tool and MCP-specific frontmatter is intentionally ignored for now. Legacy `.github/agents/` and `~/.copilot/agents/` installs are still detected for management.

#### Per-harness model and reasoning effort

A subagent definition can pin a model and reasoning effort per harness with four flat frontmatter keys (quoted or unquoted values are both accepted):

| Key | Used by | Effect |
|-----|---------|--------|
| `model` | Claude Code / Copilot | Read natively from the Markdown; never emitted into the Codex TOML (Claude model ids are not valid Codex models) |
| `effort` | Claude Code | Claude Code reasoning effort (`low`, `medium`, `high`, `xhigh`, `max`); also the Codex fallback when `codex_effort` is absent |
...
Definitions without any of these keys produce exactly the same TOML as before, so existing installs are not flagged as needing an update.
